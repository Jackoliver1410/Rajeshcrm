...
app.post("/api/auth/forgot-password", ah(async (req, res) => {
  ...
  if (user) {
    ...
    for (const row of existing.filter((r) => r.user_id === user.id && !r.used)) {
    ...
    }).catch((err) => console.error("[auth] forgot-password send failed:", err.message));
  }
  res.json({ ok: true });
}));
...
app.post("/api/users", requireLogin, requireRole("admin"), ah(async (req, res) => {
  ...
  const row = await store.insert("users", {
    ...
    password: bcrypt.hashSync(password, 8), role, manager_id: managerId,
  });
...
app.post("/api/ai/draft-sequence", requireLogin, ah(async (req, res) => {
  ...
  try {
    ...
    const result = await ai.generateOutreachSequence({
      ...
      notes: [account?.notes, contact.iq_notes].filter(Boolean).join("\n"),
      context,
...
app.post("/api/ai/draft-stage", requireLogin, ah(async (req, res) => {
  ...
  try {
    ...
    const text = await ai.generateStageEmail({
      provider: active.provider,
      apiKey: active.api_key,
      stage,
      contactName: `${contact.first_name} ${contact.last_name}`,
...
app.post("/api/contacts", requireLogin, ah(async (req, res) => {
  const { first_name, last_name, email, phone, title, linkedin_url, sales_navigator_url, account_id } = req.body || {};
...
app.post("/api/contacts/import", requireLogin, ah(async (req, res) => {
  ...
  const importable = ["phone", "title", "linkedin_url", "sales_navigator_url", "status"];
...
app.put("/api/integrations/:provider", requireLogin, requireRole("admin"), ah(async (req, res) => {
  ...
  if (active && !isApollo) {
    // Only one active AI provider at a time for the drafting/brief features.
    // Apollo isn't part of that exclusivity — it's a different capability
    // entirely, not an interchangeable text-generation backend.
    for (const r of rows) {
  ...
  else if (active !== undefined) patch.active = Boolean(active);

  const row = existing
    ? await store.update("integrations", existing.id, patch)
    : await store.insert("integrations", { active: false, ...patch });
