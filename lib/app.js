...
app.get("/api/dashboard/stats", requireLogin, ah(async (req, res) => {
  ...
  // Month-by-month breakdown for the current year, same "leave" vs "wfh"
  // split as above -- powers the Dashboard's Leaves/WFH card when a rep
  // clicks through for the fuller picture instead of just this month's
  // total. 12 entries, Jan first, zero-filled for months with nothing.
  const sumMyDaysForMonth = (wfh, monthNum) => myLeaveRequests
    .filter((r) => isWfhType(r.leave_type_id) === wfh && String(r.start_date || "").slice(0, 4) === yearPrefix && Number(String(r.start_date || "").slice(5, 7)) === monthNum)
    .reduce((s, r) => s + (r.days || 0), 0);
  const myLeaveByMonth = (wfh) => Array.from({ length: 12 }, (_, i) => sumMyDaysForMonth(wfh, i + 1));
  const myLeave = {
    leave: { month_days: sumMyDays(true, false), year_days: sumMyDays(false, false), by_month: myLeaveByMonth(false) },
    wfh: { month_days: sumMyDays(true, true), year_days: sumMyDays(false, true), by_month: myLeaveByMonth(true) },
  };
...
app.delete("/api/leave-requests/:id", requireLogin, ah(async (req, res) => {
  const reqRow = await store.find("leave_requests", req.params.id);
  if (!reqRow) return res.status(404).json({ error: "Not found" });

  // Withdrawing your own not-yet-approved request (still pending, or
  // already rejected) -- from the "My requests" panel everyone sees on
  // their own Leaves/WFH page. No manager/admin role and no password
  // re-entry needed for this path: it's always your own record, and never
  // one that already deducted from a leave balance (only "approved" does
  // that -- see the decision route above), so there's nothing destructive
  // enough here to warrant either gate.
  if (reqRow.user_id === req.user.id && reqRow.status !== "approved") {
    await store.remove("leave_requests", req.params.id);
    return res.json({ ok: true });
  }

  if (!["admin", "hr", "manager", "sub_admin"].includes(req.user.role)) {
...
// Strips punctuation and common corporate suffixes (Inc, LLC, Pvt Ltd, etc.)
// so "ACCELQ" and an Apollo record filed as "AccelQ, Inc." still compare
// equal, without loosening the match enough to accept an unrelated company.
function normalizeCompanyName(s) {
  return String(s || "")
    .toLowerCase()
    .replace(/[.,]/g, "")
    .replace(/\b(inc|incorporated|llc|ltd|limited|corp|corporation|co|company|technologies|technology|tech|pvt|private|group)\b/g, "")
    .replace(/\s+/g, " ")
    .trim();
}
...
app.post("/api/accounts/lookup-company", requireLogin, ah(async (req, res) => {
  ...
  if (apolloKey) {
    try {
      ...
      // Apollo's name search (q_organization_name) is a fuzzy keyword match,
      // not an exact lookup -- for an org it doesn't have on file, the
      // top-ranked "result" can be a completely unrelated company that just
      // shares some letters (e.g. searching "ACCELQ" ranking "AccelQuant"
      // first). Blindly taking companies[0] here used to fill the whole form
      // with a confidently wrong company's website/LinkedIn/etc. Only accept
      // a name-search result when it's actually the same company once
      // punctuation/suffixes are normalized away; otherwise treat it as no
      // match (below) and let the Claude web-search fallback research the
      // name the rep actually typed, unpoisoned by a bad anchor.
      const match = domain
        ? companies.find((c) => domainsMatch(c.website, domain))
        : companies.find((c) => normalizeCompanyName(c.name) === normalizeCompanyName(raw));
      if (match) {
