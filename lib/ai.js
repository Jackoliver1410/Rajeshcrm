...
function buildStagePrompt({ stage, contactName, title, company, industry, accountNotes, contactIQ, senderCompanyInfo, priorEmails, context }) {
  ...
  return [
    ...
    (priorEmails || []).length
      ...
      : "",
    `Respond in exactly this format: a line starting with "Subject: " followed by a short subject line (under 8 words, no placeholder brackets), then a blank line, then the email body. Keep the body under ${isFollowup ? 90 : 120} words, no placeholder brackets, sign off with just "Best," (no name -- the sender adds their own).`,
  ].filter(Boolean).join("\n");
}

// The model is asked (see buildStagePrompt above) to lead with a
// "Subject: ..." line, a blank line, then the body -- this pulls those
// apart so the app can store/show the subject and body separately. Falls
// back to treating the whole reply as the body (empty subject) if a
// provider ever ignores the format, so a drafting call still succeeds.
function parseSubjectAndBody(raw) {
  const trimmed = String(raw || "").trim();
  const match = trimmed.match(/^subject:\s*(.+?)\s*\r?\n+([\s\S]*)$/i);
  if (match) {
...
async function generateStageEmail({ provider, apiKey, stage, contactName, title, company, industry, accountNotes, contactIQ, senderCompanyInfo, priorEmails, context }) {
  ...
  const raw = await entry.call(apiKey, prompt);
  if (!raw) throw new Error("The provider returned an empty response");
  return parseSubjectAndBody(raw);
}
