...
async function getListContacts({ apiKey, listId, page }) {
  // 100 is Apollo's documented per-page ceiling for this endpoint -- was 25
  // (Apollo's default when per_page is omitted), which meant a list of any
  // real size needed 4x as many "Load more" round trips as necessary to see
  // it all.
  const data = await apolloFetch(apiKey, "/contacts/search", {
    label_ids: [listId], page: page || 1, per_page: 100,
  });
  const people = firstNonEmptyArray(data.contacts, data.people).map((p) => {
    ...
    return {
      ...
      // Same fields searchPeople() pulls for the live-search table, added
      // here too so a saved-list import shows exactly as much detail as a
      // live search does instead of a trimmed-down subset.
      location: [p.city, p.state, p.country].filter(Boolean).join(", "),
      phone: p.phone_numbers?.[0]?.raw_number || "",
    };
  });
...
async function getListCompanies({ apiKey, listId, page }) {
  // See the per_page comment in getListContacts above -- same reasoning.
  ...
  const companies = firstNonEmptyArray(data.accounts, data.organizations).map((o) => ({
    ...
    phone: o.phone || "",
    linkedin_url: o.linkedin_url || "",
    // Matches searchCompanies()'s shape (added here for the same reason as
    // location above -- a saved-list import shouldn't show less than a live
    // search does).
    employees: o.estimated_num_employees ?? null,
    revenue: o.organization_revenue_printed || o.annual_revenue_printed
      || (o.annual_revenue ? `$${Math.round(o.annual_revenue / 1e6)}M` : "") || "",
